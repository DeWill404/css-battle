...
function minify(code) {
  ...
  code = (function() {
      ...
      if (style_pos == -1) {
        ...
        return code.slice(0, end_closing) + code.slice(style_pos, code.length - 8);
      }
    })();
  ...
  code =(function () {
      let temp_code = code;
      // Extract classnames
      let classNames = temp_code.match(/\.[\w-]+/g).map(c => c.slice(1));
      classNames.sort();
      classNames.reverse();
      ...
      while( (matcher = classname_reg.exec(temp_code)) != null ) {
        ...
        let classname_end = matcher.index+ 7 + temp_code.slice(matcher.index+7).indexOf("'");
